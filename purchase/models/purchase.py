...
help="Depicts the date where the Quotation should be validated and converted into a purchase order.")
    ...
    added_notes = fields.Text('Added Notes')

    calculate_all = fields.Float(compute="calc", store=True)
    get_log_in_user = fields.Float(compute='_log_data', store=True)
    get_user_department = fields.Float(compute='_get_department_of_log_user', store=True)
    get_user_log_in_department = fields.Char('Department')
    log_in_details = fields.Char('Log in Details')
    total_all_here = fields.Float('Total')
    num2words_total = fields.Char('Total Number in Words')
    sample_field = fields.Integer('sample')
    sample_field_compute = fields.Float()
    ...
    def calc(self):
        ...
        total_compute = 0
        for x in order_line:
            total = x.price_unit * x.product_qty
            total_compute = total_compute + total
            print(total_compute)
            num2words_total_word = num2words(total_compute) + " " + 'Only'
            print(num2words_total_word)
            self.num2words_total = num2words_total_word
            self.total_all_here = total_compute

    def _log_data(self):
        for rec in self.user_id:
            details = rec.name
            print(details)
            self.log_in_details = details

    def _get_department_of_log_user(self):
        find = self.env['hr.employee']
        for rec in find:
            test = rec.find.search([('user_id', '=', self.user_id.id)])
            for x in test:
                test1 = x.department_id.name
                self.get_user_log_in_department = test1
                print(test1)
            print(test)
    ...
    def test_log_data(self):
        for rec in self:
            remove_comma = re.sub(',', '', str(rec.sample_field))
            print('My new string: {}'.format(remove_comma))
            self.sample_field = remove_comma
            # test = self.requi




    # computed_total = fields.Float(string='Total', compute='_compute_total')
    #
    # def _compute_total(self):
    #     for rec in self:
    #         print(rec.currency_rate * 1)
        # for rec in self:
        #     print(rec.price_unit * rec.product_qty)
        # num2word_of_total = fields.Char(compute='num2_words')
        #
        # total_of_all = fields.Float('Total')
    ...
    #         #         * x.product_qty
    #         # print(total)
    #         # self.total_of_all = total
    # @api.depends('total_of_all')
    # def num2_words(self):
    #     for rec in self:
    #         word_here = num2words(rec.total_of_all)
    #         print(word_here)
    #         return word_here
